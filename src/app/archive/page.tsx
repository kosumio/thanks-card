...
import { ChevronLeft, ChevronRight, Heart, Clock, Flame, Trophy, MapPin, Award } from "lucide-react";
...
export default function ArchivePage() {
  const availableMonths = useMemo(
    () => getAvailableMonths(thanksCards),
    []
  );
  ...
  const handleToggleReact = (cardId: string, currentlyReacted: boolean) => {
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() => handleToggleReact(card.id, hasReacted)}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[var(--color-warm-100)]">
                  ...
                  <th className="py-2 px-3 font-bold text-center text-[var(--color-warm-700)]">合計</th>
                </tr>
              </thead>
              <tbody>
                {locationStats.map((stat) => {
                  const catData = locationTab === "received" ? stat.catReceived
                    : locationTab === "sent" ? stat.catSent
                    : stat.catHearts;
                  const total = locationTab === "received" ? stat.totalReceived
                    : locationTab === "sent" ? stat.totalSent
                    : stat.totalHearts;
                  return (
                    <tr key={stat.location} className="border-b border-[var(--color-warm-50)] last:border-b-0">
                      <td className="py-2 px-3 font-medium text-[var(--color-warm-700)]">{stat.location}</td>
                      {CATEGORIES.map((cat) => (
                        <td key={cat.value} className="py-2 px-1.5 text-center text-[var(--color-warm-600)]">
                          {catData[cat.value] || "-"}
                        </td>
                      ))}
                      <td className="py-2 px-3 text-center font-bold text-[var(--color-primary)]">{total}</td>
                    </tr>
                  );
                })}
                {/* 合計行 */}
                <tr className="bg-[var(--color-warm-50)]">
                  <td className="py-2 px-3 font-bold text-[var(--color-warm-800)]">合計</td>
                  {CATEGORIES.map((cat) => {
                    const colTotal = locationStats.reduce((sum, s) => {
                      const d = locationTab === "received" ? s.catReceived
                        : locationTab === "sent" ? s.catSent
                        : s.catHearts;
                      return sum + d[cat.value];
                    }, 0);
                    return (
                      <td key={cat.value} className="py-2 px-1.5 text-center font-bold text-[var(--color-warm-700)]">
                        {colTotal || "-"}
                      </td>
                    );
                  })}
                  <td className="py-2 px-3 text-center font-bold text-[var(--color-primary)]">
                    {locationStats.reduce((sum, s) => {
                      return sum + (locationTab === "received" ? s.totalReceived
                        : locationTab === "sent" ? s.totalSent
                        : s.totalHearts);
                    }, 0)}
                  </td>
                </tr>
