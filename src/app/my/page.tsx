...
export default function MyPage() {
  ...
  const { isUnread, markAsRead, setReceivedCardIds } = useReadStatus();

  const { received, sent } = useMemo(
    () => (currentUser ? getCardsForUser(currentUser.id) : { received: [], sent: [] }),
    [currentUser]
  );
  ...
  useEffect(() => {
    if (received.length > 0) {
      setReceivedCardIds(received.map((c) => c.id));
    }
  }, [received, setReceivedCardIds]);

  // マイページを開いて2秒後に既読にする（新着感を味わう時間）
  useEffect(() => {
    if (tab !== "received" || received.length === 0) return;
    const unreadIds = received.filter((c) => isUnread(c.id)).map((c) => c.id);
    if (unreadIds.length === 0) return;
    const timer = setTimeout(() => {
      markAsRead(unreadIds);
    }, 2000);
    return () => clearTimeout(timer);
  }, [tab, received, isUnread, markAsRead]);
  ...
  const badges = useMemo((): Badge[] => {
    ...
    // 月ごとにグループ化
    const monthMap = new Map<string, typeof thanksCards>();
    ...
    function checkRanking(
      ...
      const prefix = categoryLabel ? `${monthLabel} ${categoryLabel}` : monthLabel;
      ...
      const heartRanking = Object.entries(heartsByPerson)
        .filter(([, v]) => v > 0)
        .sort((a, b) => b[1] - a[1]);
      const heartRank = heartRanking.findIndex(([id]) => id === currentUser!.id);
      if (heartRank >= 0 && heartRank < 3) {
        result.push({ label: `${prefix} 注目度`, rank: heartRank + 1, type: "hearts", category: categoryLabel });
      }
      ...
      if (recvRank >= 0 && recvRank < 3) {
        result.push({ label: `${prefix} もらった数`, rank: recvRank + 1, type: "received", category: categoryLabel });
      }
      ...
      if (sentRank >= 0 && sentRank < 3) {
        result.push({ label: `${prefix} 贈った数`, rank: sentRank + 1, type: "sent", category: categoryLabel });
      }
    }
  ...
  const badgeIcon = (type: Badge["type"]) => {
    if (type === "hearts") return <Heart className="w-3 h-3" fill="currentColor" />;
    ...
    <AuthGuard>
      ...
      <div className="flex bg-[var(--color-warm-100)] rounded-xl p-1 mb-4">
        ...
        >
          ...
          もらった
          {received.filter((c) => isUnread(c.id)).length > 0 && (
            <span className="min-w-[18px] h-[18px] flex items-center justify-center px-1 rounded-full bg-[var(--color-primary)] text-white text-[9px] font-bold">
              {received.filter((c) => isUnread(c.id)).length}
            </span>
          )}
        </button>
        <button
